; define some symbols (constants)
   maxLvlBufferSize equ 5000    ;;; should equal width times height plus a newline character at the end of each line.
...
section .data
    ...
    printMenuFrame db "+----------------------+",0x0A,0x00
    printPause     db "|       [Paused]       |",0x0A,0x00
    printContinue  db "| Spacebar to continue |",0x0A,0x00
    printQtoQuit   db "|   Press q to quit    |",0x0A,0x00
    printHowToPlay db "| W A S D for controls |",0x0A,0x00
    printGameOver  db "|     [GAME OVER]      |",0x0A,0x00
...
_GameTick:
    ...
    cmp AL, 'd'
    je _keyRight
    cmp AL, ' '   ; press spacebar to pause and unpause the game
    je _spaceBar
    cmp AL, 'q'   ; press q to exit the game
    je _exit
    
    ; freezes the assemblipede until w,a,s, or d is pressed for the first time.
    cmp edi, 0
    jl _GameLoop
    
    ; continues after initial press of the directional keys wasd
    jmp _continue
    ...
    push eax  ;;; save registers to restore after the call (since we're not using them yet, we could skip this... but that is risky)
    push ecx
    push edx
    ...
    cmp AL, ' '   ; unpauses game if spacebar is pressed again
    je _GameLoop
    cmp AL, 'q'   ; quits the game if q is pressed
    je _exit
    
    jmp _spaceBarLoop   ; loops around to _spaceBar if nothing is pressed
    ...
    _skipWrapTailIndex:    
       ...
       ; game over if assemblipede touches characters below
       cmp byte [eax], '|'
       je _gameOver
       cmp byte [eax], '+'
       je _gameOver
